...
import { AzureCsvFileInput } from '../azure/blob/AzureCsvFileInput';
import { AzureOptionsHandler } from '../../common/AzureOptionsHandler';
...
export class CsvFileInput extends BaseCoreComponent {
  constructor() {
    ...
    const form = {
      ...
      fields: [
        ...
        ...AzureOptionsHandler.getAZBFields(),
        {
          type: "file",
          label: "File path",
          id: "filePath",
  ...
  public generateComponentCode({ config, outputName }): string {
    ...
    if (FileUtils.isWildcardInput(config.filePath)) {
      ...
      if (config.fileLocation === "azb") {
        code += FileUtils.getADLSFilePaths(config.filePath, storageOptionsString, outputName);
        code += FileUtils.generateConcatCode(outputName, "read_csv", optionsString, true);
      } 
      else {
